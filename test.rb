...
def calculate(mass, path)
  ...
  path.each do |step|
    if step[0] == :launch
        fuel += ((mass * step[1] * 0.042) - 33).floor
        additional_fuel = fuel
          while additional_fuel > 0
              additional_fuel = ((additional_fuel * step[1] * 0.042) - 33).floor
      ...
      fuel += ((mass * step[1] * 0.033) - 42).floor
      additional_fuel = fuel
        while additional_fuel > 0
          additional_fuel = ((additional_fuel * step[1] * 0.033) - 42).floor
              if additional_fuel >= 0
                fuel += additional_fuel
              end
        end
...
puts calculate(28801, [[:launch, 9.807], [:land, 1.62], [:launch, 1.62], [:land, 9.807]])
puts calculate(75432, [[:launch, 9.807], [:land, 1.62], [:launch, 1.62], [:land, 3.711], [:launch, 3.711], [:land, 9.807]])